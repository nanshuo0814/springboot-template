...
import com.nanshuo.springboot.model.dto.user.UserQueryRequest;
import com.nanshuo.springboot.model.dto.user.admin.AdminUpdateUserInfoRequest;
...
public class AdminController {
    ...
    public ApiResponse<Long> addUser(@RequestBody AdminAddUserRequest adminAddUserRequest) {
        return ApiResult.success(userService.addUser(adminAddUserRequest));
    }
    ...
    @Check(checkParam = true, checkAuth = UserConstant.ADMIN_ROLE)
    public ApiResponse<Long> updateUser(@RequestBody AdminUpdateUserInfoRequest adminUpdateUserInfoRequest) {
        User user = new User();
        ...
        return ApiResult.success(user.getUserId());
    }
    ...
    public ApiResponse<UserSafetyVO> getUserVOById(@CheckParam(required = NumberConstant.TRUE_VALUE, alias = "用户id") Long id) {
        ApiResponse<User> response = getUserById(id);
        User user = response.getData();
        return ApiResult.success(userService.getUserVO(user));
    }
    ...
    @CheckAuth(mustRole = UserConstant.ADMIN_ROLE)
    public ApiResponse<Boolean> userPasswordReset(@RequestBody @ApiParam(value = "用户id", required = true) @CheckParam(required = NumberConstant.TRUE_VALUE, alias = "用户id") Long userId) {
