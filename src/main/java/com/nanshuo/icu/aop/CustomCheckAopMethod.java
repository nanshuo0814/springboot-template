...
import com.nanshuo.icu.utils.RegexUtils;
...
public class CustomCheckAopMethod {
    ...
    private static void validateParams(Method method, Object[] arguments) throws BusinessException {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (isBasicType(type) || notCustomRequestType(type)) {
                ...
                if (checkParam != null) {
                    if (checkParam.required() == NumberConstant.DEFAULT_VALUE && checkParam.minLength() == NumberConstant.DEFAULT_VALUE && checkParam.maxLength() == NumberConstant.DEFAULT_VALUE && checkParam.minValue() == NumberConstant.DEFAULT_VALUE && checkParam.maxValue() == NumberConstant.DEFAULT_VALUE && checkParam.regex().equals(VerifyParamRegexConstant.NONE)) {
    ...
    private static void checkBasicValue(Object value, CheckParam checkParam) throws BusinessException {
        ...
        if (!ObjectUtils.isEmpty(value)) {
            ...
            if (value instanceof Integer || value instanceof Long) {
                ...
                // 如果设置了正则表达式，并且值不符合要求
                if (checkParam.required() != NumberConstant.DEFAULT_VALUE && !StringUtils.isEmpty(checkParam.regex()) && !RegexUtils.matches(checkParam.regex(), valueString)) {
        ...
        try {
            ...
            for (Field field : fields) {
                ...
                if (checkParam != null) {
                    ...
                    if (checkParam.required() == NumberConstant.FALSE_ZERO_VALUE && checkParam.minLength() == NumberConstant.DEFAULT_VALUE && checkParam.maxLength() == NumberConstant.DEFAULT_VALUE && checkParam.regex().equals(VerifyParamRegexConstant.NONE)) {
                        continue;
                    }
    ...
    public void interceptor(JoinPoint joinPoint) throws BusinessException {
        try {
            ...
            // 如果存在 @Verify 注解并且需要进行参数验证，则执行参数验证
