package com.nanshuo.icu.service.impl;
...
import com.nanshuo.icu.utils.redis.RedisUtils;
...
import static com.nanshuo.icu.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public int deleteUser(Long id, User loginUser) {
        ...
        //}
        // 管理员之间也不能删除
        if (loginUser.getUserRole().equals(UserConstant.ADMIN_ROLE) && deleteUser.getUserRole().equals(UserConstant.ADMIN_ROLE)) {
    ...
    public long addUser(UserAddRequest userAddRequest,User loginUser) {
        ...
        // 超级管理员权限
        if (userRole.equals(UserConstant.ADMIN_ROLE) && !loginUser.getUserEmail().equals(superAdminEmail)) {
    ...
    public int updateUserInfo(UserUpdateRequest userUpdateRequest, HttpServletRequest request) {
        ...
        boolean isLoginSuperAdmin = loginUser.getUserEmail().equals(superAdminEmail);
        // 超级管理员除了他本身都可以修改其他用户角色（包括管理员）
        if (isLoginSuperAdmin && oldUser.getUserEmail().equals(superAdminEmail) && !newRole.equals(UserConstant.ADMIN_ROLE)) {
